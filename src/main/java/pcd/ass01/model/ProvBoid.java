...
public class ProvBoid {
    private P2d pos;
    private V2d vel;
    private final Lock lock;

    public ProvBoid(P2d pos, V2d vel) {
        this.pos = pos;
        this.vel = vel;
    ...
    public void update(ProvModel model) {
        lock.lock();
        try {
            List<ProvBoid> nearbyBoids = getNearbyBoids(model);
    ...
    private List<ProvBoid> getNearbyBoids(ProvModel model) {
        List<ProvBoid> list = new ArrayList<>();
        for (ProvBoid other : model.getBoids()) {
            if (other != this) {
    ...
    private V2d calculateAlignment(List<ProvBoid> nearbyBoids, ProvModel model) {
        double avgVx = 0, avgVy = 0;
        if (!nearbyBoids.isEmpty()) {
            for (ProvBoid other : nearbyBoids) {
    ...
    private V2d calculateCohesion(List<ProvBoid> nearbyBoids, ProvModel model) {
    ...
    private V2d calculateSeparation(List<ProvBoid> nearbyBoids, ProvModel model) {
        double dx = 0, dy = 0;
        int count = 0;
        for (ProvBoid other : nearbyBoids) {
            P2d otherPos = other.getPos();
